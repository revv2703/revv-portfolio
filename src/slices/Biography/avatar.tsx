...
export default function Avatar({image, className}: AvatarProps){
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            gsap.fromTo(".avatar", {scale: 1.4, opacity: 0}, {scale: 1, opacity: 1, duration: 1.25, ease: "power2.out", scrollTrigger: {trigger: component.current, start: "top 80%"}})
            window.onmousemove = (e) => {
                ...
                gsap.timeline({
                    defaults: {duration: .5, overwrite: "auto", ease: "power2.out"}
                }).to(".avatar", {
